...
import jfabrix101.billing.BillingSecurity.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    private static final String TAG = "jfabrix101-BillingService";

    /** The service connection to the remote MarketBillingService. */
    private static IMarketBillingService mService;

    private void trace(String msg) { 
    	 if (BillingConsts.DEBUG) Log.d(TAG, msg); 
    }
    ...
    abstract class BillingRequest {
        ...
        public BillingRequest(int startId) {  mStartId = startId; }

        public int getStartId() { return mStartId; }
        ...
        public boolean runIfConnected() {
            if (mService != null) {
                try {
                    mRequestId = run();
                    trace(" +-- " + getClass().getSimpleName() + ".runIfConnected() - " +
                    		" requestType: " + getClass().getSimpleName() +   
                    		" assigned requestID: " + mRequestId);
                    if (mRequestId >= 0) {
        ...
        protected void onRemoteException(RemoteException e) {
        	trace(getClass().getSimpleName() + ".onRemoteException() - remote billing service crashed");
            mService = null;
        }
        ...
        protected void responseCodeReceived(ResponseCode responseCode) {
        	trace(getClass().getSimpleName() + ".responseCodeReceived() - responseCode=" + responseCode);  
        }

        protected Bundle makeRequestBundle(String method) {
            Bundle request = new Bundle();
            request.putString(BillingConsts.BILLING_REQUEST_METHOD, method);
            request.putInt(BillingConsts.BILLING_REQUEST_API_VERSION, 2);
        ...
        protected void logResponseCode(String method, Bundle response) {
            ResponseCode responseCode = ResponseCode.valueOf(response.getInt(BillingConsts.BILLING_RESPONSE_RESPONSE_CODE));
            trace( getClass().getSimpleName() + ".logResponseCode() - " + method + " received " + responseCode.toString());
        }
    ...
    class CheckBillingSupported extends BillingRequest {
        ...
        protected long run() throws RemoteException {
        	trace("Sending message CHECK_BILLING_SUPPORTED");
            Bundle request = makeRequestBundle("CHECK_BILLING_SUPPORTED");
            if (mProductType != null) {
                request.putString(BillingConsts.BILLING_REQUEST_ITEM_TYPE, mProductType);
                trace("      +-- BillingRequestItemType: " + mProductType);
            }
            Bundle response = mService.sendBillingRequest(request);
            int responseCode = response.getInt(BillingConsts.BILLING_RESPONSE_RESPONSE_CODE);
            if (BillingConsts.DEBUG) {
                Log.d(TAG, "CheckBillingSupported response code: " +
                        ResponseCode.valueOf(responseCode));
            }
            boolean billingSupported = (responseCode == ResponseCode.RESULT_OK.ordinal());
            BillingHandler.checkBillingSupportedResponse(billingSupported, mProductType);
    ...
    public class RequestPurchase extends BillingRequest {
        ...
        @Override
        protected long run() throws RemoteException {
        	trace("Sending message REQUEST_PURCHASE");
        	Bundle request = makeRequestBundle("REQUEST_PURCHASE");
            request.putString(BillingConsts.BILLING_REQUEST_ITEM_ID, mProductId);
            request.putString(BillingConsts.BILLING_REQUEST_ITEM_TYPE, mProductType);
            
            // Note that the developer payload is optional.
            if (mDeveloperPayload != null) {
                request.putString(BillingConsts.BILLING_REQUEST_DEVELOPER_PAYLOAD, mDeveloperPayload);
            }
            Bundle response = mService.sendBillingRequest(request);
            PendingIntent pendingIntent = response.getParcelable(BillingConsts.BILLING_RESPONSE_PURCHASE_INTENT);
            if (pendingIntent == null) {
            	trace(" +-- RequestPurchase.run() - Error with requestPurchase. pendingIntent is null");
                return BillingConsts.BILLING_RESPONSE_INVALID_REQUEST_ID;
            }

            Intent intent = new Intent();
            BillingHandler.buyPageIntentResponse(pendingIntent, intent);
            return response.getLong(BillingConsts.BILLING_RESPONSE_REQUEST_ID, BillingConsts.BILLING_RESPONSE_INVALID_REQUEST_ID);
        }

        @Override
        protected void responseCodeReceived(ResponseCode responseCode) {
        	trace("RequestPurchase.responseCodeReceived() - productID:" + mProductId + ", responceCode:" + responseCode);
            BillingHandler.responseCodeReceived(BillingService.this, this, responseCode);
        }
    }
    ...
    public class ConfirmNotifications extends BillingRequest {
        ...
        protected long run() throws RemoteException {
        	String ids = "";
        	for (String _id : mNotifyIds)  ids += _id + ", ";
        	if (ids.length() > 2) ids = ids.substring(0, ids.length() -2);
        	trace("Sending message CONFIRM_NOTIFICATIONS for ids = {" + ids + "}");
            Bundle request = makeRequestBundle("CONFIRM_NOTIFICATIONS");
            request.putStringArray(BillingConsts.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    class GetPurchaseInformation extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            trace("Sending message GET_PURCHASE_INFORMATION");
            
            Bundle request = makeRequestBundle("GET_PURCHASE_INFORMATION");
            request.putLong(BillingConsts.BILLING_REQUEST_NONCE, mNonce);
            request.putStringArray(BillingConsts.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    public class RestoreTransactions extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            mNonce = BillingSecurity.generateNonce();
            trace("Sending message RESTORE_TRANSACTIONS");
        ...
        @Override
        protected void responseCodeReceived(ResponseCode responseCode) {
        	trace("RestoreTransactions.responseCodeReceived() - responceCode = " + responseCode);
            BillingHandler.responseCodeReceived(BillingService.this, this, responseCode);
        }
        
        @Override
        public String toString() { return "" + mNonce;  }
    }

    // ---
    // ---
    // --- Inizio definizione dei metodi del servizio
    // ---
    // ---
    
    
    public BillingService() { super(); }

    public void setContext(Context context) { attachBaseContext(context); }
    ...
    public void onStart(Intent intent, int startId) {
    	if (intent == null) { Log.w(TAG, "onStart() - intent was null"); return; }
    ...
    public void handleCommand(Intent intent, int startId) {
        ...
        trace(" +-- handleCommand() - IntentAction:" + action + ", startId:" + startId);
        
        String baseActionName = BillingHelper.getAppPackageInfo(this, getClass()).packageName;
        if ((baseActionName + BillingConsts.ACTION_CONFIRM_NOTIFICATION).equals(action)) {
            String[] notifyIds = intent.getStringArrayExtra(BillingConsts.NOTIFICATION_ID);
            confirmNotifications(startId, notifyIds);
        } else if ((baseActionName + BillingConsts.ACTION_GET_PURCHASE_INFORMATION).equals(action)) {
            String notifyId = intent.getStringExtra(BillingConsts.NOTIFICATION_ID);
            getPurchaseInformation(startId, new String[] { notifyId });
        } else if (BillingConsts.VENDING_ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
        ...
        } else if (BillingConsts.VENDING_ACTION_RESPONSE_CODE.equals(action)) {
            long requestId = intent.getLongExtra(BillingConsts.INAPP_REQUEST_ID, -1);
            int responseCodeIndex = intent.getIntExtra(BillingConsts.INAPP_RESPONSE_CODE, ResponseCode.RESULT_ERROR.ordinal());
    ...
    private boolean bindToMarketBillingService() {
        try {
            trace("bindToMarketBillingService() - binding to Market billing service");
            
            boolean bindResult = bindService(
                    new Intent(BillingConsts.MARKET_BILLING_SERVICE_ACTION),
            ...
            if (bindResult) {
                ...
                Log.e(TAG, "bindToMarketBillingService() - Could not bind to service.");
            }
        } catch (SecurityException e) {
            Log.e(TAG, "bindToMarketBillingService() - Security exception: " + e);
        }
        return false;
    }
     ...
     * receives an intent with the action {@link BillingConsts#VENDING_ACTION_NOTIFY}.
     ...
     * in an intent with the action {@link BillingConsts#VENDING_ACTION_PURCHASE_STATE_CHANGED}.
    ...
    /**
     * Extension Point
     * Estendendo dal service e ridefinendo questo metodo si possono fare delle 
     * operazioni custom (tipo download dei contenuti) prima di definire acquistato
     * un bene e restituire la notifica al market.
     * Se l'operazione non va a buon fine (exception o return false) il messaggio di
     * nuovo acquisto (o refund) continua ad essere inviato dal market perchè non e'
     * ancora stato inviato il messaggio di CONFIRM
     * 
     * @param purchaseEvent - Informazioni sull'acquisto (o refund)
     * @return
     */
    protected boolean processBillingPurchase(VerifiedPurchase purchaseEvent) throws Exception {
    	return true;
    }
    
    /**
     * Verifies that the data was signed with the given signature, and calls
     * {@link BillingHandler#purchaseResponse(Context, PurchaseState, String, String, long)}
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        ...
        purchases = BillingSecurity.verifyPurchase(this, signedData, signature);
        if (purchases == null) return;

        ArrayList<String> notifyList = new ArrayList<String>();
        
        // Per ogni acquisto verificato si prepara ad inviare una notifica
        // al market (in modo che non ne invii un altro)
        for (VerifiedPurchase vp : purchases) {
        	       	
        	boolean canProcess = false;
        	try {
        		canProcess = processBillingPurchase(vp);
        	} catch (Exception e) {
        		trace("Can't process the purchase : " + vp.productId + " with state " + vp.purchaseState);
        		continue;
        	}
        	if (!canProcess) {
        		trace("Blocked purchase item " + vp.productId);
        		return;
        	}
        	
            if (vp.notificationId != null) notifyList.add(vp.notificationId);
            
            //if (vp.productId.startsWith("android.test.")) vp.productId = "androidTestItem";
            
            // Memorizza l'acquisto sul database (la scrittura avviene in un thread separato)
            BillingHandler.purchaseResponse(this, vp.purchaseState, vp.productId,
                    vp.orderId, vp.purchaseTime, vp.developerPayload);
        }
        
        // Manda un unico messaggio al market con gli ID delle notifiche
    ...
    private void checkResponseCode(long requestId, ResponseCode responseCode) {
        ...
        if (request != null) {
        	trace("Menaging RESPONSE_CODE for message type " + request.getClass().getName());
        	RequestPurchase requestPurchaseInstance = null;
        	if (request instanceof RequestPurchase) {
        		requestPurchaseInstance = (RequestPurchase) request;
        		trace(" +--- requestType:" + (request.getClass().getSimpleName()) +
        				", productID:" + requestPurchaseInstance.mProductId + 
            			", requestID:" + requestId + 
            			", responseCode: " + responseCode +
            			", developerPayload: " + requestPurchaseInstance.mDeveloperPayload);
        	} else {        	
        		trace(" +--- checkResponseCode() - requestType:" + (request.getClass().getSimpleName()) +
        			", requestID:" + requestId + 
        			", responseCode: " + responseCode);
        	}
            request.responseCodeReceived(responseCode);
        } else {
        	trace("checkResponseCode() - Unable to pair response with request (requestId:" + requestId + ")");
        }
    ...
    private void runPendingRequests() {
        ...
        // stop it now.
        if (maxStartId >= 0) {
            trace("runPendingRequests() - stopping service, startId: " + maxStartId);
            stopSelf(maxStartId);
        }
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        trace("onServiceConnected() - Billing service connected");
    ...
    public void onServiceDisconnected(ComponentName name) {
        trace("onServiceConnected() - Billing service disconnected");
        mService = null;
    }
