...
public class BillingHandler {
    ...
    /**
     * This is a static instance of {@link BillingObserver} that the
     ...
     */
    public static synchronized void register(BillingObserver observer) {
     ...
     */
    public static synchronized void unregister(BillingObserver observer) {
    ...
    /**
     * Notifies the application of purchase state changes. The application can offer an item
     * for sale to the user via {@link BillingService#requestPurchase(String)}. 
     * The BillingService calls this method after it gets the response. Another way this method
     * can be called is if the user bought something on another device running this same app. 
     * Then Android Market notifies the other devices that the user has purchased an item, 
     * in which case the BillingService will also call this method. 
    ...
    public static void purchaseResponse(
        ...
        // first.
    	Log.d(TAG, "purchaseResponse() - ProductId:" + productId + ", purchaseState:" + purchaseState);
    	new Thread(new Runnable() {
            @Override
            public void run() {
                BillingDatabase db = new BillingDatabase(context);
                db.insertOrder(orderId, productId, purchaseState, purchaseTime, developerPayload);
                db.close();
     ...
     * are handled in {@link BillingSecurity#verifyPurchase(String, String)}.
    ...
    public static void responseCodeReceived(Context context, RequestPurchase request, ResponseCode responseCode) {
    	Log.d(TAG, "responseCodeReceived() - " + request.getClass().getSimpleName() + ", responseCode:" + responseCode);
        if (sPurchaseObserver != null) {
