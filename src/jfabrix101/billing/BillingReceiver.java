...
public class BillingReceiver extends BroadcastReceiver {
    ...
    private static final String TAG = "jfabrix101-BillingReceiver";
     ...
     * the {@link BillingHandler}.
     */
    @Override
    public void onReceive(Context context, Intent intent) {
    	String action = intent.getAction();
    	
        if (BillingConsts.VENDING_ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
        	trace("Received message: PURCHASE_STATE_CHANGED ");
        ...
        } else if (BillingConsts.VENDING_ACTION_NOTIFY.equals(action)) {
        	trace("Received message: IN_APP_NOTIFY ");
            String notifyId = intent.getStringExtra(BillingConsts.NOTIFICATION_ID);
            if (BillingConsts.DEBUG) Log.i(TAG, "notifyId: " + notifyId);
            notify(context, notifyId);
        } else if (BillingConsts.VENDING_ACTION_RESPONSE_CODE.equals(action)) {
        	trace("Received message: RESPONSE_CODE ");
    ...
    private void purchaseStateChanged(Context context, String signedData, String signature) {
    	trace(" +--- purchaseStateChanged() - ... ");
        Intent intent = new Intent(BillingConsts.VENDING_ACTION_PURCHASE_STATE_CHANGED);
    ...
    private void notify(Context context, String notifyId) {
    	trace(" +--- notify() - notifyId: " + notifyId);
    	String baseActionName = BillingHelper.getAppPackageInfo(context, getClass()).packageName;
        Intent intent = new Intent(baseActionName + BillingConsts.ACTION_GET_PURCHASE_INFORMATION);
        intent.setClass(context, BillingService.class);
    ...
    private void checkResponseCode(Context context, long requestId, int responseCodeIndex) {
    	trace(" +--- checkResponseCode() - requestId:" + requestId + ", responseCode:" + responseCodeIndex);
        Intent intent = new Intent(BillingConsts.VENDING_ACTION_RESPONSE_CODE);
        intent.setClass(context, BillingService.class);
